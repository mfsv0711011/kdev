<template>
    <button
        :disabled="isDisabled"
        v-bind="$attrs"
        class="h-10 sm:h-12 xl:h-[54px] rounded-10 text-white transition-all outline-gray"
        :class="{ 'bg-lightGray cursor-not-allowed': isDisabled, 'bg-purple kButtonShadow cursor-pointer': !isDisabled }"
    >
        <svg v-if="isLoading" class="animate-spin size-5 text-white mx-auto" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 16 16">
...
<style scoped>
.kButtonShadow:hover {
    box-shadow: 0 4px 8px 2px #7F56D9aa;
}
</style>
